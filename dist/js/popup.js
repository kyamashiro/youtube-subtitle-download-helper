...
function download() {
    ...
    getRequest(language_url)
        ...
        const subtitle = new subtitle_1.default(response);
        if (format === 'csv') {
...
class Subtitle {
    ...
    getVtt(filename) {
        let converter = __webpack_require__(/*! json-2-csv */ "./node_modules/json-2-csv/src/converter.js");
        const options = {
            delimiter: {
                wrap: '',
                field: '',
    ...
    removeReservedCharacters(filename) {
        return filename.replace(/[\/\\:;|=,."']/g, '');
    }
    convertXMLtoText(transcript) {
        const lines = transcript
            .replace('<?xml version="1.0" encoding="utf-8" ?><transcript>', '')
            .replace('</transcript>', '')
            .split('</text>')
    ...
    mergeTime(dateStartStr, dateEndStr) {
