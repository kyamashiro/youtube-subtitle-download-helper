...
import type {
  ...
  TextAline,
  VttAline,
} from "@/shared/types/aline";
import { createConverter, FileFormat } from "./converterFactory";

let xml: string;
beforeAll(async () => {
  xml = await fs.readFile(
    "src/lib/converter/testData/Grit the power of passion and perseverance  Angela Lee Duckworth - 英語.xml",
    "utf-8",
  );
});
...
let csv: CsvAline[];
beforeAll(async () => {
  const data = await fs.readFile("src/lib/converter/testData/csv.json", "utf-8");
...
let lrc: LrcAline[];
beforeAll(async () => {
  const data = await fs.readFile("src/lib/converter/testData/lrc.json", "utf-8");
...
let srt: SrtAline[];
beforeAll(async () => {
  const data = await fs.readFile("src/lib/converter/testData/srt.json", "utf-8");
...
let vtt: VttAline[];
beforeAll(async () => {
  const data = await fs.readFile("src/lib/converter/testData/vtt.json", "utf-8");
...
let txt: TextAline[];
beforeAll(async () => {
  const data = await fs.readFile("src/lib/converter/testData/txt.json", "utf-8");
