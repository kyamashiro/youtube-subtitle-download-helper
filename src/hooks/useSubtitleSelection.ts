...
export function useSubtitleSelection(props: UseSubtitleSelectionProps) {
  ...
  const [userSelectedTrack, setUserSelectedTrack] = createSignal<string | undefined>();
  const [userSelectedFormat, setUserSelectedFormat] = createSignal<FileFormat | undefined>();

  // Computed Default Format
  const defaultFormat = (): FileFormat => {
    const userSettings = settings();
    if (userSettings?.defaultFormat) {
       return userSettings.defaultFormat as FileFormat;
    }
    return "srt";
  };
  ...
  const defaultTrack = (): string | undefined => {
    ...
    if (userSettings.defaultLanguage && userSettings.defaultLanguage !== "auto") {
